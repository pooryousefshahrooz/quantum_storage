...
def CPLEX_resource_cinsumption_minimization(network,work_load,life_time,iteration,cyclic_workload,storage_capacity):
...
def minimizing_resource_consumption(each_network_topology_file,spike_means,num_spikes,experiment_repeat,storage_node_selection_schemes,fidelity_threshold_ranges,cyclic_workload,storage_capacities,given_life_time_set,distance_between_users):
    
    for spike_mean in spike_means:
        for network_topology,file_path in each_network_topology_file.items():
            results_file_path = 'results/results_minimizing_swappings.csv'
            inventory_utilization_results_file_path = 'results/inventory_utilization_swapping.csv'
            import pdb
            ...
            for i in range(experiment_repeat):
                ...
                #                     print('network.each_request_virtual_paths',network.each_request_virtual_paths)
                                    ...
                                    for life_time in given_life_time_set:
                                        try:
                                            ...
                                            try:
                                                ...
                                                objective_value,each_inventory_per_time_usage = CPLEX_resource_cinsumption_minimization(network,work_load,life_time,i,cyclic_workload,storage_capacity)
                                            except ValueError:
                                                print(ValueError)
                                                #pass
                        ...
                        #                     import pdb
                        #                     
                                            print("storage_capacity %s life time %s the objective value for %s storage nodes and %s paths between each pair of nodes is %s"%
                                                  (storage_capacity,life_time,number_of_storages,num_paths,objective_value))
                                            #pdb.set_trace()
                                            #time.sleep(5)
                                            #if objective_value>=0:
...
minimizing_resource_consumption(each_network_topology_file,spike_means,num_spikes,experiment_repeat,storage_node_selection_schemes,fidelity_threshold_ranges,cyclic_workload,storage_capacities,given_life_time_set,distance_between_users)
