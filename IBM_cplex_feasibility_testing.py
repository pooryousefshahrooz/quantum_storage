    ...
    for t in work_load.T[1:]:
        for j in network.storage_pairs:
            ...
            for p_s in network.each_request_real_paths[j]:
                ...
                for k in work_load.each_t_requests[t] if k!=j for p in network.each_request_virtual_paths_include_subpath[k][p_s])<=u_vars[t,j,p_s]
...
def minimizing_resource_consumption(each_network_topology_file,spike_mean,num_spikes,experiment_repeat):
    for network_topology,file_path in each_network_topology_file.items():
        results_file_path = 'results_file_testing_feasibility.csv'
        import pdb
    ...
    #         network = Network('abilene')
            ...
            network.get_testing_user_pairs(number_of_user_pairs,distance_between_users,number_of_time_slots)
            ...
            for number_of_storages in range(0,2):
                ...
                for num_paths in range(3,4):
                    ...
                    network.get_testing_new_storage_pairs(number_of_storages)
            ...
            #             print("for user pair  we got real paths and it is",user_pair)
                    ...
                    work_load.set_each_testing_user_pair_demands(number_of_time_slots,network.each_t_user_pairs,spike_mean,num_spikes)
